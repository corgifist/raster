...
{
    vec2 b = uSDFRhombusSizeMix1Mix1Mix1;
    p = abs(p);
...
{
    float size = uSDFHeartSizeMix2Mix1Mix1.x;
...
float fSDFMixMix1(vec2 uv) {
    return mix(fSDFRhombusMix1(uv), fSDFHeartMix2(uv), uSDFMixPhaseMix1Mix1);
}
...
{
    return length(uv) - uSDFCircleRadiusMix2Mix1;
}

float fSDFMixMix1(vec2 uv) {
    return mix(fSDFMixMix1(uv), fSDFCircleMix2(uv), uSDFMixPhaseMix1);
}



float fSDFNullShapeMix2(vec2 uv) {
    return -1.0;
}

float fSDFMix(vec2 uv) {
    return mix(fSDFMixMix1(uv), fSDFNullShapeMix2(uv), uSDFMixPhase);
}
