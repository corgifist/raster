...
ImRect ed::Link::GetBounds() const
{
    if (m_IsLive)
    {
        ...
        if (m_StartPin->m_ArrowSize)
        {
            ...
            const auto p1 = curve.m_Points[0] - start_dir * m_StartPin->m_ArrowSize;const auto min = ImMin(p0, p1);
        ...
        if (m_EndPin->m_ArrowSize)
        {
            ...
            const auto p1 = curve.m_Points[curve.m_NumPoint - 1] + end_dir * m_EndPin->m_ArrowSize;const auto min = ImMin(p0, p1);
            const auto max = ImMax(p0, p1);
            auto arrowBounds = ImRect(min, ImMax(max, min + ImVec2(1, 1)));
            bounds.Add(arrowBounds);
        }
