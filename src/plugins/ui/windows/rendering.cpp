...
namespace Raster {
    ...
    Json RenderingUI::AbstractSerialize() {
        return {};
    }

    void RenderingUI::AbstractLoad(Json t_data) {
        
    }

    void RenderingUI::AbstractRender() {
        if (!open) {
            Layouts::DestroyWindow(id);
        }
        ImGui::SetNextWindowSize(ImVec2(600, 600), ImGuiCond_FirstUseEver);
        if (ImGui::Begin(FormatString("%s %s###%i", ICON_FA_IMAGE, Localization::GetString("RENDERING").c_str(), id).c_str(), &open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_MenuBar | ImGuiWindowFlags_NoScrollWithMouse)) {
