...
namespace Raster {
    ...
    Json AudioBusesUI::AbstractSerialize() {
        return {};
    }

    void AudioBusesUI::AbstractLoad(Json t_data) {
        
    }

    void AudioBusesUI::AbstractRender() {
        if (!open) {
            Layouts::DestroyWindow(id);
        }
        ImGui::SetNextWindowSize(ImVec2(300, 700), ImGuiCond_FirstUseEver);
        if (ImGui::Begin(FormatString("%s %s###%i", ICON_FA_VOLUME_HIGH, Localization::GetString("AUDIO_BUSES").c_str(), id).c_str(), &open)) {
