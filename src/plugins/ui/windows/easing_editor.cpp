...
namespace Raster {
    ...
    Json EasingEditorUI::AbstractSerialize() {
        return {};
    }

    void EasingEditorUI::AbstractLoad(Json t_data) {
        
    }

    void EasingEditorUI::AbstractRender() {
        if (!open) {
            Layouts::DestroyWindow(id);
        }
        ImGui::SetNextWindowSize(ImVec2(300, 700), ImGuiCond_FirstUseEver);
        if (ImGui::Begin(FormatString("%s %s###%i", ICON_FA_BEZIER_CURVE, Localization::GetString("EASING_EDITOR").c_str(), id).c_str(), &open)) {
